...
class PolicyDistParamBatch:
    ...
    def to_pdparam(self) -> PolicyDistParam:
        return PolicyDistParam(self.discrete_pdparams, self.continuous_pdparams)
        
@dataclass(frozen=True)
class PolicyDistParamSeq:
    """
    Policy distribution parameter sequence (`pdparam_seq`) data type.
    It's usually generated by the recurrent network. 
    Note that this `pdparam_seq` must be valid to the policy you currently use.
    
    When the action type is discrete, it is generally either logits or soft-max distribution. \\
    When the action type is continuous, it is generally mean and standard deviation of gaussian distribution.

    Args:
        discrete_pdparams (List[Tensor]): `(num_seq, seq_len, *discrete_pdparam_shape)` x `num_discrete_branches`
        continuous_pdparams (List[Tensor]): `(num_seq, seq_len, *continuous_pdparam_shape)` x `num_continuous_branches`
    """
    discrete_pdparams: Tuple[torch.Tensor] = field(default_factory=tuple)
    continuous_pdparams: Tuple[torch.Tensor] = field(default_factory=tuple)
    
    def to_pdparam(self) -> PolicyDistParam:
        """Convert to `PolicyDistParam`."""
        return PolicyDistParam(self.discrete_pdparams, self.continuous_pdparams)
