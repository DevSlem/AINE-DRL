...
class SAC(Agent):
    ...
    def train(self):
        for _ in range(self.config.epoch):
            ...
            self._train_v_net(exp_batch)
            self._train_q_net(exp_batch)
            self._train_actor(exp_batch)
    ...
    def _train_v_net(self, exp_batch: ExperienceBatchTensor):
        """objective function J_V: 0.5 * (V(s) - Q(s,a) + log(pi(a|s)))^2"""
        with torch.no_grad():
            # compute log(pi(a|s))
            pdparam = self.network.actor.forward(exp_batch.obs)
            dist = self.policy.get_policy_distribution(pdparam)
            action_log_prob = dist.log_prob(exp_batch.action)
        ...
        soft_value_loss = F.mse_loss(v_pred, min_q_pred - action_log_prob)
    ...
    def _train_q_net(self, exp_batch: ExperienceBatchTensor):
        """objective function J_Q: 0.5 * (Q(s,a) - q_hat(s,a))^2"""
        with torch.no_grad():
            # compute q_hat
            not_terminated = 1.0 - exp_batch.terminated
            v_target = self.v_target_net.forward(exp_batch.next_obs)
            q_hat = exp_batch.reward + not_terminated * self.config.gamma * v_target 
    ...
    def _train_actor(self, exp_batch: ExperienceBatchTensor):
        ...
        policy_kl_loss = (action_log_prob - min_q_pred).mean()
